...
/// the different updates are merged by data fusion.
///
...
/// would take every symbol's market data down with the first bad topic.
///
...
pub fn subscription_frames(symbols: &[String]) -> Vec<String> {
    symbols
        .iter()
        .map(|symbol| {
            let args = vec![
...
impl PublicStream {
        ...
        BybitError: From<S::Error>,
    {
        ...
        for frame in subscription_frames(&pending) {
    ...
    /// the one call the venue's silence depends on — is driven by the tests below rather than
    /// only modelled by them.
...
mod tests {
    ...
    fn stream_with_capacity(registered: &[&str], wake_ups: usize) -> Fixture {
        ...
        (
            PublicStream::new(ev_tx, symbols.clone(), symbol_rx),
            symbols,
            symbol_tx,
            ev_rx,
        )
    ...
    /// connect. The broadcast cannot deliver them: `Connector::register` sends
    ...
    /// Bybit fails the *whole* batch for one unknown topic and then keeps the socket open,
    /// answering pings, so the symbols in it have
    ...
    /// one, the first bad topic would take every symbol's market data down, and the connector
    /// would sit connected and subscribed to nothing.
    #[test]
    fn each_symbol_gets_its_own_batch() {
        ...
        for (frame, symbol) in frames.iter().zip(["BTCUSDT", "ETHUSDT"]) {
            ...
            assert_eq!(args.len(), 5, "four depths plus publicTrade: {frame}");
            assert!(
                args.iter()
