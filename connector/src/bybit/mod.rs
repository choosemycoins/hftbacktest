...
impl Connector for Bybit {
    ...
    ///
    /// Reports [`SweepOutcome::NotImplemented`], **not `None`** (Finding 2): this backend has
    /// an order path, so orders it placed can be resting — the stop is told so honestly. It
    /// stays non-fatal (§4.7); `None` is reserved for a backend that can hold nothing.
    fn sweep(
        &self,
        symbols: Vec<String>,
        reason: SweepReason,
