...
async fn run() {
    ...
    //
    // SW1: fold the task into a `SweepOutcome`, so the exit code can tell "cancelled" from
    // "ran but may have left orders resting". A panic (`Ok(Err(_))`) or a timeout is `Failed`
    // — orders may be on the venue — not a silent success. `None` here means no sweep task ran
    // at all (disabled, nothing registered, or a backend with no order path).
