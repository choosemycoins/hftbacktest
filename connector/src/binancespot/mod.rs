...
impl Connector for BinanceSpot {
    ...
    /// [`SweepOutcome::NotImplemented`], **not `None`** (SW1): the order path can leave orders
    /// resting, so this is a documented, non-fatal gap — distinct from a backend with no order
    /// path at all (`None`).
    fn sweep(
        &self,
        symbols: Vec<String>,
...
mod tests {
    ...
    fn register_lowercases_the_symbol_and_survives_having_no_listener() {
        let mut connector = BinanceSpot::build_from(
