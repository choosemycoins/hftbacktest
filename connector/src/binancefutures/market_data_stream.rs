...
impl MarketDataStream {
    ...
    where
        ...
        BinanceFuturesError: From<S::Error>,
    {
        ...
        info!(
            symbols = pending.len(),
            ?pending,
            "Subscribed to the Binance Futures market data streams."
        );
        Ok(())
    }
    ...
    /// stays open and keeps answering pings. That is the silent-failure shape of `AGENTS.md`
    /// §4.1 on this venue, so the bots are told and their `error_handler` decides what it is
    /// worth.
        ...
        match &result.error {
            Some(error) => {
                error!(
                    ?error,
                    id = %result.id,
                    "Binance Futures refused a subscribe request. That symbol gets no market \
                     data on this connection, and it will not be retried."
                );
    ...
    async fn serve<S, R>(&mut self, write: &mut S, read: &mut R) -> Result<(), BinanceFuturesError>
    where
        S: SinkExt<Message> + Unpin,
        BinanceFuturesError: From<S::Error>,
        ...
        loop {
            select! {
                ...
                message = read.next() => match message {
                    ...
                    Some(Err(error)) => {
                        // Named rather than `from`: with `BinanceFuturesError: From<S::Error>`
                        // in scope the conversion is ambiguous.
                        return Err(BinanceFuturesError::Tunstenite(error));
                    }
                    None => {
                        return Err(BinanceFuturesError::ConnectionInterrupted);
                    }
...
mod tests {
    ...
    fn stream_with_capacity(registered: &[&str], wake_ups: usize) -> Fixture {
        ...
        (
            MarketDataStream::new(
                BinanceFuturesClient::new("https://example.invalid", "", ""),
                ev_tx,
                symbols.clone(),
                symbol_rx,
            ),
    ...
    fn subscribed(sink: &RecordingSink<BinanceFuturesError>) -> Vec<serde_json::Value> {
        sink.sent
            .iter()
    ...
    async fn every_registered_symbol_is_subscribed_at_connect() {
        ...
        let mut sink = RecordingSink::<BinanceFuturesError>::default();
        ...
        assert!(
            matches!(error, BinanceFuturesError::ConnectionInterrupted),
            "{error:?}"
        );
