...
pub struct Order {
    ...
    /// (`order_last_filled_qty`) two fields above is that, and the two sit close enough that
    /// a one-identifier slip reinstates the exact double-count E5 exists to close.
    /// [`CumulativeFilled`] is what stops it: there is no route from here into
    /// `Order::exec_qty` (`AGENTS.md` §4.6, §4.11).
    #[serde(rename = "z")]
    #[serde(deserialize_with = "from_str_to_cumulative_filled")]
    pub order_filled_accumulated_qty: CumulativeFilled,
    #[serde(rename = "L")]
    #[serde(deserialize_with = "from_str_to_f64")]
    pub last_filled_price: f64,
    // #[serde(rename = "N")]
