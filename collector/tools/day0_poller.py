...
def load_state(text: str | None) -> State:
    ...
    Прецедент positions-поллера. Плата за re-bootstrap здесь мала по построению: env
    будет собран из тех же символов и совпадёт по байтам, значит ни записи, ни рестарта
    не будет — продублируется максимум один алерт.
    """
    if not text:
        return State()
    try:
