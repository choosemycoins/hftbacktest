...
* Список адресов порождается `collector/tools/build_operator_watchlist.py`.
  **Ни сам список, ни его провенанс в репозитории не лежат и лежать не должны.**
  Это 60 адресов живых участников рынка вместе со связкой суб-аккаунт → мастер,
  то есть готовый инструмент деанонимизации, а форк публичный. Файл
  `operators_addrs.json.example` в релизе — обезличенная рыба формата (три
  очевидно поддельных адреса); настоящий список и провенанс живут только на
  рабочей машине, в `/home/storage/hft-scratch/firewall/`, и внесены в
  `.git/info/exclude`. Оператор ставит настоящий файл прямо в
  `/opt/hft-collector/etc/operators_addrs.json` — релиз его не перезаписывает.
  Историческая справка: обе копии были однажды закоммичены и вычищены из истории
  переписыванием до первого push (22.08.2026).
