...
**Смежное, тоже починено:** отвергнутый venue-ом subscribe больше не тонет в `debug!`. Bybit смотрит `success`/`ret_msg` в `OpResponse`, Binance — новое поле `error` в `msg::stream::Result` (без него отказ парсился в `result: None` и был неотличим от подтверждения); оба логируют `error!` и публикуют `ErrorKind::CriticalConnectionError`. Само сообщение биржи остаётся в логе и **не** едет в payload: `LiveEvent` ограничен 512 байтами (§2), а oversize валит publish-таск. Повторной попытки на этой связи нет намеренно — батч детерминированный, второй раз его отвергнут так же, а рейт-лимит потратится.

Приватный стрим Bybit этой проблемы не имел: он перечитывает `self.symbols` на subscribe-ack (`private_stream.rs:100–151`).

**Что НЕ закрыто:** отвергнутая подписка не пере-запрашивается до реконнекта, и *транзиентный* отказ (рейт-лимит) от постоянного здесь не отличается — `req_id` не пер-символьный, сказать «кого разметить обратно» нечем; чинится тем, что в `req_id` кладётся символ. `sweep`/`shutdown` у этих трёх бэкендов по-прежнему no-op (§4.7).
...
| `collector/tools` (Python) | 345 pytest (337 прогон + 8 skip): `test_quality_report.py` (123), `test_build_dataset.py` (99), `test_backtest_first.py` (123). Запуск: `.venv/bin/pytest collector/tools/` — тулинг Фаз 2–4 `docs/design-multi-venue-collection.md` |
