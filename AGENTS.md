...
**Что НЕ закрыто:** отвергнутая подписка не пере-запрашивается до реконнекта. Символ теперь известен, но *транзиентный* отказ (рейт-лимит `10006`) от постоянного (`error:handler not found`) не отличается, а разметить обратно осмысленно только транзиентный: детерминированный батч второй раз отвергнут так же, и повтор потратит рейт-лимит на тот же ответ. Различать пришлось бы по `ret_msg`/коду — отдельная задача, и до неё оба случая восстанавливаются на реконнекте, который переподписывает всё заново. `sweep`/`shutdown` у этих трёх бэкендов по-прежнему no-op (§4.7).
