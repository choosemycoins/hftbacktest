...
- **Штатный SIGINT коннектора всегда оканчивается паникой** (`unwrap` на `SendError` в `publish`, когда publish-receiver уже дропнут) и exit 1 — супервизор не отличит стоп от краша. Быстрый «не паниковать» сделал бы зомби-стрим при нештатной смерти publish-таска, так что настоящий фикс — упорядоченный shutdown в `connector/src/main.rs` (общий для всех бэкендов). Хорошая отдельная задача.
