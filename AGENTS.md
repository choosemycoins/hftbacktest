...
**Не подхвачено (в этой правке вне scope, но должно быть поправлено) — обе позиции закрыты 2026-08-02:**
...
- ~~`docs/design-multi-venue-collection.md` — запрет `PartialFillExchange` «до фикса» в трёх местах~~ — **починено 2026-08-02.** Три места (не 360/584/630, как было записано здесь: файл с тех пор вырос, реальные — Фаза 4, «Открытые решения» и «Что оказалось не так, как задумывалось»). Бюллетень Фазы 4 теперь говорит, что запрет снят вместе с багом, и **рядом с ним** — второй буллет с остаточными дырами (многоуровневый тейкер, частично исполненный IOC/Market, `FlatPerTradeFeeModel` за каждое исполнение), чтобы «разрешена» не читалось как «моделирует всё». Прозу пинит только структурный тест манифеста (пункт выше) — дизайн-нота тестов не имеет, так что проверяется грепом: `grep -n PartialFillExchange docs/design-multi-venue-collection.md` не должен давать ни одной формы «запрещена/до фикса» в настоящем времени.
...
- **«Отказ хенд-оффа фатален» имеет ровно одно исключение — сам wind-down** (`queue::Stopping`, 2026-08-02). `wind_down` объявляет остановку **до** отмены таски сбора и до закрытия хопов, и с этого момента `FatalTx::raise` не поднимает сигнал, а считает отказ (`debug!` вместо `error!`); итоговое число печатает `main`. Причина не косметическая: продюсеры, до которых отмена не достаёт, — это норма, а не край (`pump` спавнит читателя сокета отдельной таской, REST-снапшотчики держат по клону `writer_tx`), поэтому **каждый** `systemctl stop` с непустой очередью писал `error!: a data hand-off failed; the collector will stop`. Код выхода при этом оставался 0 и `_meta` не писался — врал журнал, а не данные, но оператор, видящий эту строку на каждой штатной остановке, перестаёт её читать. Fail-closed держится тем, что защёлка ставится **после** того, как цикл уже выбрал код выхода, и `main` больше никогда не читает фатальный канал: подавляется отчёт, который всё равно ничего не изменил бы. Отказ остаётся отказом — `Tx::send` по-прежнему возвращает `Err`, запись не принимается. Пины: `a_refusal_is_fatal_until_the_wind_down_makes_it_expected` (обе половины — до защёлки фатально, после считается) и `a_hand_off_the_wind_down_cannot_reach_is_refused_without_reporting_a_fault` (на настоящей обвязке `pump`). Прежний пин `a_producer_still_delivering_when_the_loop_stops_is_written_not_refused` этого не видел вовсе: его продюсер — сама таска сбора, без единого ребёнка.
- **Wind-down режет сокетный хоп, а не сливает его. Не закрыто.** Таска сбора **и есть** `pump`, владеющий приёмником сокетного хопа, поэтому `abort` в `stop_collecting` уничтожает всё, что в этом хопе лежало, — кадры, которые читателю уже отчитались как принятые. Замерено: из 500 принятых 244 ушли с отменой, молча (дропнутый приёмник ничего не отказывает, счётчик отказов остаётся нулевым). Это тот же контракт «принятое не теряется», сломанный на хоп выше, и он **старше** wind-down: до него `main` просто возвращался и уничтожал оба хопа сразу. Обычно потеря нулевая — парсер это копия строки и роутинг, хоп пуст, пока лента его не обгоняет, — но обгон ровно то состояние, в котором остановка и случается: 2026-07-29 этот хоп налился до 16 384. Починка — каскад, а не заплатка: останавливаться должен **читатель**, тогда `pump` видит дропнутый отправитель, дочитывает остаток в писательский хоп и возвращается сам. Это путь отмены через пять бэкендовых `keep_connection` — отдельная задача со своими тестами.
