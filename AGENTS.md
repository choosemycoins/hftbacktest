...
- **Единицы времени на бэкенде Lighter — микросекунды у венью, наносекунды в `Order`/`LiveEvent`; починено 2026-08-09.** `apply_order_update` писал `transaction_time_us` в `Order::exch_timestamp` без множителя, а `LiveEvent::Position.exch_ts` — через `timestamp_micros()`. Три дня в проде это молчало по структурной причине: внутри одного бэкенда все значения в одной и той же неверной единице, ряд монотонен, и anti-reorder-страж `LiveBot::process_event` работает. Смертельно это становится, когда в то же поле пишет второй источник в правильной единице (так делают оба Binance на ветке протухания ордера) — тогда бот навсегда перестаёт применять апдейты по ордеру, который торгуется. Множитель — **1_000**, и скопировать `* 1_000_000` от bybit/Binance (у них источник в миллисекундах) значит промахнуться на три порядка в другую сторону **с переполнением `i64`**. Разбор и пины — `docs/design-lighter-connector.md` §4.12.
- **Исходящая латентность Lighter разложена по стадиям, строка `"Lighter placement timing."`** (`info!`, одна на размещение). Поля `ipc_us`, `queue_us`, `nonce_us`, `sign_us`, `send_us` — grep-таргеты оператора и потому контракт; арифметика в чистой `slot::Outbound::spans` и пиннута. Зачем: ИЗМЕРЕНО (n = 60 719, трое суток) — обратная нога 135 мс, исходящая **913 мс**, то есть вся латентность на пути туда, а лид сигнала Binance ИЗМЕРЕН как 450–800 мс. Сайдкар-подписант из подозреваемых выбыл — ИЗМЕРЕНО 4,0 мс на подпись, 0,4 % от 913. Оговорки и то, чего эта инструментовка не видит, — `docs/design-lighter-connector.md` §4.12a.
