...
- **proptest** для слоя 2 — новая dev-зависимость `hftbacktest`; первый арендатор — учёт филлов §4.6 (тождество позиции над произвольной последовательностью Filled/PartiallyFilled/reject/ack/amend), плюс `debug_assert!(Σexec_qty == qty - leaves_qty)` внутри `apply_fill`, чтобы регрессия delta-vs-cumulative падала в *каждом* тесте, который филлит.
