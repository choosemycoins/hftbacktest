...
- **Штатный конвертер падал на нашей же записи USD-M — починено 2026-08-27.** Строка `premiumIndex` (нет `data`, нет `code`, нет `T`) проваливалась в ветку REST-снапшота `py-hftbacktest/hftbacktest/data/utils/binancefutures.py` и валила конвертацию `KeyError: 'T'` на первой из ~121 таких строк в сутки на символ. То есть **каждый** USD-M файл, записанный с 2026-07-28, не конвертировался вообще, на всех инстансах, а `collector/README.md` утверждал обратное. Снапшот теперь опознаётся по наличию `bids`/`asks`. Урок общий: «пропускаем известных чужаков» — не гард, гард — «принимаем только то, что несёт нужное поле».
- **Разрыв цепочки `pu` у Binance UM чинится REST-снапшотом, и этот ремонт может не состояться.** Отказ (рейт-лимит собственного дросселя или отказ площадки) оставляет книгу неверной до следующего разрыва, а кадры по обе стороны выглядят идеально. Замерено по журналу Токио: 19.08 за две минуты 139 ремонтов отклонил дроссель — и в `_meta` об этом не было ничего. С 2026-08-27 несостоявшийся ремонт пишется записью `depth_repair_failed`, а гейт делает из неё жёлтую находку с именем символа; `fetch_depth_snapshot` наконец проверяет HTTP-статус, поэтому тело 429 больше не попадает в `.gz` как снапшот. Дроссель `Throttler::new(100)` по-прежнему **попроцессный**, а лимит — по IP: семь UM-инстансов делят 2400 весов/мин, и общего бюджета у них нет.
- **Профиль гейта задаётся per-instance.** `gate-run.sh` читает `GATE_PROFILE` из env самого инстанса (не исполняя файл). `book-v1` требует у семейств Binance все три потока (`bookTicker`, `trade`, `depthUpdate`): для инстанса, который пишут ради книги, её потеря обязана быть красной, иначе таймер, эскалирующий только RED, промолчит. Под дефолтным `mode-a-v1` стакан остаётся опциональным — мода A читает только тач.
- **Сбор USDC-перпов Binance (`binancefuturesum-usdc`, с 2026-08-27).** 24 USDC-символа + NEOUSDT, ~0.85 ГБ/сут (измерено на самом инстансе). Смысл: публичной книги для USDⓈ-M нет — `data.binance.vision` отдаёт `bookTicker` только для BTCUSDC/ETHUSDC за 2024-01…04, у листнутых позже директория пуста. Отбор и оба его исключения (ZECUSDC как якорь калибровки, NEOUSDT как контроль пары) объяснены в `collector/deploy/binancefuturesum-usdc.env.example`. Комиссию мейкера (ровно 0 с 2024-04-03, промо без срока) сторожит `collector/tools/commission_watch.py` — почасовой опрос `/fapi/v1/commissionRate` **с машины оператора**, потому что на хосте сбора нет ни одного секрета и заводить первый ради этого не надо.
